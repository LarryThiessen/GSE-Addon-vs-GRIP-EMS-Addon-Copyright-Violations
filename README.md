...
| `evidence/RELEASE-DELTA-ANALYSIS-2026-08-26.md` | **Answers "maybe he just hasn't got to it."** Release-by-release change counts, v2.3.18→v2.4.8: **11,266 lines across 311 file-revisions and 16 new files** in the period the claim was live. In v2.4.6 the export and share files got no functional edit at all. In **v2.4.8 he rewrote 551 lines of `GRIPExport.lua` and 341 of `Transmission.lua`** — the files that build and send an export — to add an author-confirmation flow, and `PlatformID` and `HelpURL` are **still at zero references**. Sets out why difficulty is not an available answer: the identifier is already in the payload his own website emits, the refusal point exists and he built it, and he shipped a comparable gate in a day. Carries a dated correction of an earlier working note. |
